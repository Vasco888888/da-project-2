...
/**
 * @brief Recursive backtracking function to explore all possible combinations
 * of pallets.
 *
 ...
 */
void backtrack(const vector<Pallet> &pallets, int index, int currentWeight,
               int currentProfit, int capacity, vector<Pallet> &currentSet,
               vector<Pallet> &bestSet, int &bestProfit, int &bestWeight) {
  ...
  if (currentWeight + p.weight <= capacity) {
    ...
    backtrack(pallets, index + 1, currentWeight + p.weight,
              currentProfit + p.profit, capacity, currentSet, bestSet,
              bestProfit, bestWeight);
    currentSet.pop_back();
  }

  // Exclude current pallet
  backtrack(pallets, index + 1, currentWeight, currentProfit, capacity,
            currentSet, bestSet, bestProfit, bestWeight);
}

/**
 * @brief Solves the knapsack problem using a brute-force approach.
 *
 * @param instance The knapsack problem instance containing pallets and
 * capacity.
 * @return A vector of selected pallets that maximize profit while respecting
 * constraints.
 */
vector<Pallet> bruteForceKnapsack(const Instance &instance) {
  ...
  backtrack(instance.pallets, 0, 0, 0, instance.capacity, currentSet, bestSet,
            bestProfit, bestWeight);
