...
/**
 * @brief Solves the knapsack problem using a hybrid approach combining multiple
 * strategies.
 *
...
vector<Pallet> hybridKnapsack(const Instance &instance) {
  ...
  int subsetSize = greedySet.size() +
                   max(1, static_cast<int>(instance.pallets.size() * 0.30));

  vector<Pallet> sortedByRatio = instance.pallets;
  sort(sortedByRatio.begin(), sortedByRatio.end(),
  ...
  for (const auto &p : sortedByRatio) {
    if (find_if(candidates.begin(), candidates.end(),
                [&](const Pallet &existing) { return existing.id == p.id; }) ==
        candidates.end()) {
  ...
  backtrack(candidates, 0, 0, 0, instance.capacity, currentSet, bestSet,
            bestProfit, bestWeight);

  sort(bestSet.begin(), bestSet.end(),
       [](const Pallet &a, const Pallet &b) { return a.id < b.id; });
